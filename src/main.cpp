...
int main(int argc, char** argv) {
    ...
    options.add_options()
        ("c,config", "Path to the configuration file location", cxxopts::value<std::string>())
        ("zeek-config-location", "Overrides the default configuration location of Zeek", cxxopts::value<std::string>())
        ("i,interface", "Starts Zeek in network analysis mode on the specified interface", cxxopts::value<std::string>())
        ("f,file", "Absolute path to one pcap file", cxxopts::value<std::string>())
        ("h,help", "Print usage")
    ;
    ...
    try {
        if (initial_zeek_setup) {
            ...
            if (fs::exists(zeek_config_location)) {
                fs::copy_file(zeek_config_location, default_zeek_config_backup_location, fs::copy_options::overwrite_existing);
            }
        } else {
            spdlog::info("Restore default config");
            if (fs::exists(default_zeek_config_backup_location)) {
                fs::copy_file(default_zeek_config_backup_location, zeek_config_location, fs::copy_options::overwrite_existing);
            }
        }
